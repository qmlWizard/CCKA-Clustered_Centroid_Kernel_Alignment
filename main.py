...
for run in range(5):
    ...
    if config['training']['method'] == 'ccka':
        ...
        experiment_name = f"embedded_results/{config['dataset']['name']}_{config['training']['method']}_{config['training']['clusters']}_exp1.json"
    else:
        agent = train_model(
            kernel=kernel,
            training_data=training_data,
        ...
        experiment_name = f"embedded_results/{config['dataset']['name']}_{config['training']['method']}_{config['training']['sampling']}_exp1.json"
...
os.makedirs("embedded_results", exist_ok=True)
